...
/* ─── ★ NEW: Lazy-load heavy components that were blocking initial render ─── */
const LandingPage = dynamic(() => import('./landing-page').then(m => ({ default: m.LandingPage })), {
  loading: () => <div className="min-h-screen bg-background" />,
});
const DonationPopup = dynamic(() => import('./donation-popup').then(m => ({ default: m.DonationPopup })), {
  loading: () => null,
});
const NotificationStack = dynamic(() => import('./notification-stack').then(m => ({ default: m.NotificationStack })), {
  loading: () => null,
});
...
export function AppShell() {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'admin': return adminAuth.isAuthenticated ? <AdminPanel /> : (() => { setTimeout(() => { setAccountModalDefaultTab('admin'); setAccountModalOpen(true); setCurrentView('dashboard'); }, 0); return null; })();
